...
// Fetch data from contact_form table
$sql = "SELECT id, name, email, message FROM contact_form";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<div class="card" style="background-color: aliceblue; border:2px solid #F1C40F; border-radius:15px; padding: 15px;">';
        echo '<p>ID: ' . $row['id'] . '</p>';
        echo '<p>Name: ' . $row['name'] . '</p>';
        echo '<p>Email: ' . $row['email'] . '</p>';
        echo '<p>Message: ' . $row['message'] . '</p>';
        echo '<button class="btn btn-outline-danger mt-2" onclick="deleteMessage(' . $row['id'] . ')">Delete</button>';
        echo '</div>';
        echo '</div>';

        // Close the row and start a new row for every 3 columns
        if ($result->num_rows % 3 == 0) {
            echo '</div>';
            echo '<div class="row justify-content-center">';
        }
    }
